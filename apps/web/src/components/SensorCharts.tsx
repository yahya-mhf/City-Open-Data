  ...
  if (metrics.length === 0 && !statsLoading) {
    return (
      <div className="bg-white dark:bg-night-secondary rounded-xl shadow p-6">
        <p className="text-gray-600 dark:text-gray-300 text-center py-8">No metrics available for this sensor</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {stats.map((s) => (
              <div key={s.metric_key} className="bg-gray-50 dark:bg-night-border/50 rounded-lg p-4">
                <div className="text-xs text-gray-500 dark:text-gray-300 uppercase tracking-wide">{s.display_name}</div>
                ...
                <div key={mk.key} className="bg-gray-50 dark:bg-night-border/30 rounded-lg p-4">
                  ...
                  <ResponsiveContainer width="100%" height={200}>
                    <ComposedChart data={combined}>
                      ...
                      <XAxis
                        dataKey="time"
                        tick={{ fontSize: 9, fill: nightMode ? "#d1d5db" : "#6b7280" }}
                        interval="preserveStartEnd"
                      />
                      <YAxis tick={{ fontSize: 9, fill: nightMode ? "#d1d5db" : "#6b7280" }} />
      ...
      <div className="bg-white dark:bg-night-secondary rounded-xl shadow p-6">
        ...
        {heatmapLoading ? (
          ...
          <p className="text-gray-600 dark:text-gray-300 text-sm">No heatmap data available</p>
        ) : (
          <div className="space-y-6">
            {metrics.map((mk) => {
      ...
      <div className="bg-white dark:bg-night-secondary rounded-xl shadow p-6">
        ...
        {distLoading ? (
          ...
          <p className="text-gray-600 dark:text-gray-300 text-sm">No distribution data available</p>
        ) : (
          <div className="space-y-6">
            {metrics.map((mk) => {
                ...
                <div key={mk.key}>
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={chartData}>
                      ...
                      <XAxis
                        dataKey="range"
                        tick={{ fontSize: 8, fill: nightMode ? "#d1d5db" : "#6b7280" }}
                        interval={0}
                        angle={-45}
                        textAnchor="end"
                        height={60}
