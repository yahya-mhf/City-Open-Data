...
- Final audit pass: Removed the stale export `district` query behavior that filtered by `Sensor.type`, fixed export preview count queries to join metric definitions explicitly, and enforced public API `max_metrics` limits across `/public/v1/sensors`, `/public/v1/sensors/{id}/readings`, `/public/v1/metrics`, and `/public/v1/layers/{metric_key}`. `python -m py_compile apps/api/app/routers/public_api.py apps/api/app/api/v1/endpoints/export.py` passes.
- Final audit pass: Deleted the now-unused `ScenarioSimulator` component after confirming no imports remain. `npx tsc --noEmit` passes.
- Final audit pass: Removed the unreachable legacy city-health implementation from `main.py` after the service-layer return and hardened WebSocket cleanup so closed/stale sockets are removed without raising during disconnect or broadcast. `python -m py_compile apps/api/app/main.py apps/api/app/core/websocket_handler.py` passes.
